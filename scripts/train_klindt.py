import torch
import numpy as np
import matplotlib.pyplot as plt
from center_surround.data import load_raw_data, create_dataloaders
from center_surround.models import KlindtCoreReadout2D
from center_surround.training import train
from center_surround.utils import compute_metrics, plot_predictions_grid, plot_correlation_vs_reliability, bootstrap_reliability

# Device
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"Using device: {device}")

# Load data
data_path = "data/exp_13_data.pkl"
raw_data = load_raw_data(data_path)
dataloaders = create_dataloaders(raw_data, batch_size=32)

# Get shapes
images, responses = next(iter(dataloaders['train']))
num_neurons = responses.shape[1]
input_size = images.shape[2]
in_channels = images.shape[1]

print(f"Input: {in_channels} channels, {input_size}x{input_size}")
print(f"Neurons: {num_neurons}")

# Create model
# Create model (matching notebook parameters)
model = KlindtCoreReadout2D(
    image_size=input_size,
    image_channels=in_channels,
    kernel_sizes=[15,15],              # single layer
    num_kernels=[4],                # 4 kernels
    act_fns=['relu'],
    init_scales=[[0.0, 0.01], [0.0, 0.001], [0.0, 0.01]],
    init_kernels="gaussian:0.2",    # gaussian initialization
    num_neurons=num_neurons,
    smoothness_reg=1e-4,            # reduced
    sparsity_reg=0,                 # off
    center_mass_reg=0.4,            # added
    mask_reg=1e-3,
    weights_reg=1e-3,
    dropout_rate=0.2,
    batch_norm=True,
    bn_cent=False,
    kernel_constraint="norm",       # added
    weights_constraint="abs",       # added
    mask_constraint="abs",          # added
    final_relu=True,
    seed=42,
)
# Train
print("\nTraining...")
model = train(model, dataloaders, num_epochs=50, lr=1e-3, device=device)

# Evaluate
print("\nEvaluating...")
metrics = compute_metrics(model, dataloaders['test'], device)
print(f"Test MSE: {metrics['mse']:.4f}")
print(f"Mean Correlation: {metrics['mean_correlation']:.4f}")
print(f"Per-neuron Correlation: {metrics['correlation_per_neuron']}")

# Get predictions and targets for plotting
model.eval()
all_preds = []
all_targets = []
with torch.no_grad():
    for images_batch, responses_batch in dataloaders['test']:
        images_batch = images_batch.to(device)
        output = model(images_batch)
        all_preds.append(output.cpu().numpy())
        all_targets.append(responses_batch.numpy())

predictions = np.concatenate(all_preds, axis=0)
targets = np.concatenate(all_targets, axis=0)
correlations = metrics['correlation_per_neuron']

# Plot predictions grid
print("\nPlotting predictions...")
fig1 = plot_predictions_grid(predictions, targets, correlations)
plt.show()

# Plot correlation vs reliability (if we have trial data)
if 'responses_test_by_trial' in raw_data:
    # OpenRetinaUI expects shape: (time, neurons, trials)
    responses_by_trial = raw_data['responses_test_by_trial']
    print(f"Trial data shape: {responses_by_trial.shape}")
    
    # Check if we need to transpose
    # Expected: (time=30, neurons=8, trials=30)
    if responses_by_trial.shape[0] == 30 and responses_by_trial.shape[2] == 30:
        # Already correct shape
        pass
    elif responses_by_trial.shape[0] == 30 and responses_by_trial.shape[1] == 30:
        # Shape is (trials, time, neurons) -> transpose to (time, neurons, trials)
        responses_by_trial = responses_by_trial.transpose(1, 2, 0)
    
    print(f"Trial data shape after transpose: {responses_by_trial.shape}")
    
    reliability, reliability_ci = bootstrap_reliability(responses_by_trial)
    print(f"Reliability: {reliability}")
    
    fig2 = plot_correlation_vs_reliability(correlations, reliability)
    plt.show()
else:
    print("No trial data available for reliability computation")
