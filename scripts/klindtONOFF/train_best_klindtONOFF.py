...
from center_surround.utils import plot_kernels, plot_spatial_masks_onoff
...
# Load best hyperparameters from ON/OFF model search
...
# Using ON/OFF kernel polarity with 2 ON and 2 OFF kernels
model_config = {
    ...
    'smoothness_reg': best_params['smoothness_reg'], #0.001, #
    'sparsity_reg': 0, #best_params['sparsity_reg'],
    'center_mass_reg': 0,
    'mask_reg': best_params['mask_reg'], #1e-4,#
    'weights_reg': best_params['weights_reg'],# 1e-4,#
    'dropout_rate': 0.2,
    'batch_norm': True,
    'bn_cent': False,
...
# Create model with best parameters (ON/OFF kernel polarity)
...
# Plot ON/OFF spatial masks
print("\nPlotting ON/OFF spatial masks...")
spatial_masks_dir = f'{output_dir}/spatial_masks'
os.makedirs(spatial_masks_dir, exist_ok=True)
spatial_mask_figs = plot_spatial_masks_onoff(model, neurons_per_plot=8)
for i, fig in enumerate(spatial_mask_figs):
    fig.savefig(f'{spatial_masks_dir}/spatial_masks_onoff_{i}.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
...
model_path = f'{output_dir}/best_model_onoff.pth'
...
print(f"  - best_model_onoff.pth (trained model weights)")
...
print(f"  - spatial_masks_onoff_*.png (ON mask, OFF mask per neuron)")
