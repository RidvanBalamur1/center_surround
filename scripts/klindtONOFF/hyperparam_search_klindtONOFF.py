...
from center_surround.training import run_hyperparameter_search_onoff
...
# Run hyperparameter search for ON/OFF model
print("\nStarting hyperparameter search for KlindtONOFF model...")
print("Model architecture: 2 ON kernels + 2 OFF kernels")
print("                    ON mask pools from ON kernels only")
print("                    OFF mask pools from OFF kernels only")
print("Tuning: smoothness_reg, sparsity_reg, weights_reg, mask_reg, learning_rate, init_kernels_sigma")
results = run_hyperparameter_search_onoff(
    ...
    n_on_kernels=2,    # 2 ON kernels
    n_off_kernels=2,   # 2 OFF kernels
)
...
output_dir = f'/home/ridvan/Documents/center_surround/outputs/{data_path_file}/klindtONOFF'
